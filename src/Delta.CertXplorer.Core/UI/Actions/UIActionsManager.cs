...
public class UIActionsManager : Component, IExtenderProvider, ISupportInitialize
{
    ...
    public UIAction GetAction(Component extendee) => 
        targets.TryGetValue(extendee, out var value) ? value : null;

    public void SetAction(Component extendee, UIAction action)
    {
        if (!initializing)
        {
            ArgumentNullException.ThrowIfNull(extendee);
        ...
        if (targets.TryGetValue(extendee, out var value))
        {
            value.InternalRemoveTarget(extendee);
    ...
    protected virtual Type[] GetSupportedTypes() =>
        [typeof(ButtonBase), typeof(ToolStripButton), typeof(ToolStripMenuItem)];
    ...
    private static Control GetActiveControl(ContainerControl container)
    {
