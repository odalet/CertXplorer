...
SectionLocker::SectionLocker(CRITICAL_SECTION &critsection,char *function,int line,void* object) {
	...
	if (TryEnterCriticalSection(section)==0) {
		...
		char logBuffer[500];
		sprintf(logBuffer,"[BixVReader]Richiesto da:Function:%s,Line:%i,Object:%p,Lock:%p",Function,Line,Object,section);
...
SectionLocker::~SectionLocker() {
	if (section==ShowMe) {
		char logBuffer[500];
		sprintf(logBuffer,"[BixVReader]Posseduto da:Function:%s,Line:%i,Object:%p,Lock:%p",Function,Line,Object,section);
		OutputDebugStringA(logBuffer);
		ShowMe=NULL;
	}
